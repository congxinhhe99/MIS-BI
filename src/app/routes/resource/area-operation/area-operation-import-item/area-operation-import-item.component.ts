...
@Component({
  selector: 'app-area-operation-import-item',
  templateUrl: './area-operation-import-item.component.html',
  styleUrls: ['./area-operation-import-item.component.less'],
})
export class AreaOperationImportItemComponent implements OnInit {
  constructor(private messageService: NzMessageService, private areaOperationService: AreaOperationService) {
    //#region Button
    this.btnDownload = {
      Title: 'Tải về mẫu',
      Titlei18n: '',
    ...
    this.columnDefs = [
      ...
      { field: 'status', headerName: 'Trạng thái', cellRenderer: 'statusNameCellRender', editable: true, minWidth: 150 },
  ...
  tittle = 'Nhập dữ liệu loại hình đơn vị từ excel';
  ...
  save() {
    ...
    const promise = this.areaOperationService.createMany(data).subscribe(
      (res: any) => {
        this.isLoading = false;
        if (res.code !== 200) {
