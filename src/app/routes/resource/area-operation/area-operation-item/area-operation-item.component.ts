...
@Component({
  selector: 'app-area-operation-item',
  templateUrl: './area-operation-item.component.html',
  styleUrls: ['./area-operation-item.component.less']
})
export class AreaOperationItemComponent implements OnInit {
  @Input() type = 'add';
  @Input() item: any;
  ...
  save(isCreateAfter: boolean = false) {
    ...
    if (data.name === null || data.name === undefined || data.name === '') {
      this.isLoading = false;
      this.messageService.error(`Tên loại hình đơn vị không được để trống!`);
      return;
    }

    if (this.isAdd) {
      const promise = this.areaOperationService.create(data).subscribe(
        (res: any) => {
          this.isLoading = false;
          if (res.code !== 200) {
      ...
      const promise = this.areaOperationService.update(data).subscribe(
        (res: any) => {
          this.isLoading = false;
          if (res.code !== 200) {
            this.messageService.error(`${res.message}`);
