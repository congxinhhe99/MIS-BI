...
@Component({
  selector: 'app-district-import-item',
  templateUrl: './district-import-item.component.html',
  styleUrls: ['./district-import-item.component.less'],
})
export class DistrictImportItemComponent implements OnInit {
  constructor(
    ...
    this.columnDefs = [
      ...
      { field: 'provinceName', headerName: 'Tỉnh thành phố', sortable: true, filter: true, minWidth: 120, flex: 1 },
  ...
  tittle = 'Nhập dữ liệu quận huyện từ excel';
    ...
    reader.onload = (e) => {
      ...
      listObj.forEach((item) => {
        ...
        if ((provinceId === undefined || provinceId === null) && item.provinceName.toString().trim() !== '') {
          this.messageService.error(`Tên tỉnh thành phố ` + item.provinceName + ` là không hợp lệ`);
        }
        const model = {
          code: item.code,
          name: item.name,
  ...
  save() {
    ...
    const promise = this.districtService.createMany(data).subscribe(
      (res: any) => {
        this.isLoading = false;
        if (res.code !== 200) {
